...
public class MonsterCtrl : MonoBehaviour {
    ...
    IEnumerator CheckMonsterState()
    {
        while (!isDie)
        {
            ...
            if(dist <= attackDist)          // 공격 범위에 들어왔는지 확인
            {
                monsterState = MonsterState.attack;
            }
            else if(dist <= traceDist)      // 추적 범위에 들어왔는지 확인
            {
        ...
        while(!isDie)
        {
            switch(monsterState)
            {
                ...
                case MonsterState.trace:
                    ...
                    // 추적을 재시작
                    nvAgent.Resume();
                    // Animator의 IsAttack 변수값을 false로 설정
    ...
    void OnCollisionEnter(Collision coll)
    {
        if(coll.gameObject.tag == "BULLET")
        {
            ...
            // IsHit Trigger를 발생시키면 Any State에서 gohit으로 전이됌
